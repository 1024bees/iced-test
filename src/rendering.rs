    ...
    A: Application<Renderer = iced_graphics::Renderer<crate::renderer::Backend>> + 'static,
{
    ...
    let ss = render_winit(application, window_size);
    ss
}
    ...
    A: Application<Renderer = iced_graphics::Renderer<crate::renderer::Backend>> + 'static,
{
    ...
    let (mut compositor, mut renderer) =
        crate::renderer::window::Compositor::new(renderer_settings, Some(&window))
            .expect("FAILURE NOT ALLOWED!");
    let state = State::new(application, &window);
    let mut debug = Debug::new();
    compositor.initialize_surface(&window);
    compositor.configure_surface(window_size.0, window_size.1);
